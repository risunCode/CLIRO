<script lang="ts">
  ...
  import CliSyncInfoModal from '@/features/router/modals/CliSyncInfoModal.svelte'
  import type { CliSyncAppID, CliSyncResult, CliSyncStatus, LocalModelCatalogItem } from '@/features/router/types'
  import { CLI_SYNC_CARDS, getDefaultCliModel, groupCliModels } from '@/features/router/lib/cli-sync'
  ...
  $: if (models.length > 0) {
    for (const card of CLI_SYNC_CARDS) {
      ...
      if (!current || !stillExists) {
        selectedModels = {
          ...selectedModels,
          [card.id]: getDefaultCliModel(card.id, models)
        }
      }
    }
  }
  ...
  const syncTarget = async (targetID: CliSyncAppID): Promise<void> => {
    const model = selectedModels[targetID] || getDefaultCliModel(targetID, models)
    if (!model) {
      errors = { ...errors, [targetID]: 'No local proxy model is available for sync.' }
      return
    }
    ...
    try {
      ...
      errors = { ...errors, [targetID]: message }
    } finally {
      busyTargetID = ''
    }
  }
...
<CollapsibleSurfaceSection
  ...
  bodyClassName="api-cli-sync-body space-y-3"
>
            ...
            <div class="flex flex-1 flex-col p-2.5">
              ...
              <div class="flex flex-1 flex-col space-y-2">
                ...
                <div class="space-y-1">
                  ...
                  <select class="ui-control-input ui-control-select text-sm" value={selectedModels[tool.id]} on:change={(event) => updateModel(tool.id, event)} disabled={models.length === 0}>
                    {#if models.length === 0}
