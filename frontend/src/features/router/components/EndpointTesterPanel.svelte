<script lang="ts">
  ...
  import { ENDPOINT_PRESETS, buildEndpointTarget, buildTesterStructuredResponse, type EndpointPreset, type TesterStructuredResponse } from '@/features/router/lib/endpoint-tester'
  ...
  let requestBody = ENDPOINT_PRESETS[0].defaultBody
  let loading = false
  let status = '-'
  let responseText = ''
  let error = ''
  ...
  $: selectedEndpoint = ENDPOINT_PRESETS.find((endpoint) => endpoint.id === selectedEndpointId) || ENDPOINT_PRESETS[0]
  ...
  const applySelectedEndpoint = (event?: Event): void => {
    let nextEndpoint = selectedEndpoint
    if (event?.currentTarget instanceof HTMLSelectElement) {
      const nextEndpointID = event.currentTarget.value
      selectedEndpointId = nextEndpointID
      nextEndpoint = ENDPOINT_PRESETS.find((endpoint) => endpoint.id === nextEndpointID) || ENDPOINT_PRESETS[0]
    }
    if (nextEndpoint.method === 'POST') {
      requestBody = nextEndpoint.defaultBody
    }
  }
        ...
        <div class="api-endpoint-panel ui-panel-soft ui-panel-dashed">
          <div class="api-endpoint-controls ui-inline-controls">
            ...
            <Button
              ...
              disabled={loading || !proxyStatus?.running}
            >
